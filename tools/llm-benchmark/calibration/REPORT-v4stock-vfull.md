# Calibration report — human vs judge (judge-v4stock-vfull)

Judge run(s): Qwen/Qwen3.5-4B (bf16 cloud) (no_think=True)

- decisions compared: 63 (+1 human-na excluded, +0 unscored)
- **binary agreement (met vs not-met): 65.1%  (41/63)**
- exact-status agreement: 63.5%  (40/63)

- **with verifier: 76.2%  (48/63)** — verifier rejected 15 credits: 11 over-credits fixed, 4 correct credits destroyed

  Correct credits destroyed (recall damage):
  - headache_visit · explore_complaint
  - headache_visit · open_questions
  - rash_visit · explore_complaint
  - rash_visit · plain_language


## Per-criterion

| criterion | n | agree | human met | judge met |
|---|---|---|---|---|
| intro_self | 4 | 3/4 | 1 | 2 |
| set_tone | 4 | 3/4 | 2 | 3 |
| open_questions | 4 | 4/4 | 2 | 2 |
| explore_complaint | 4 | 3/4 | 3 | 4 |
| avoid_interrupting | 4 | 1/4 | 1 | 4 |
| what_else | 4 | 2/4 | 1 | 3 |
| explore_perspective | 4 | 2/4 | 1 | 3 |
| respond_emotion | 4 | 2/4 | 0 | 2 |
| support_respect | 4 | 1/4 | 1 | 4 |
| explain_exam | 3 | 2/3 | 2 | 3 |
| plain_language | 4 | 4/4 | 3 | 3 |
| accurate_info | 4 | 2/4 | 2 | 4 |
| shared_plan | 4 | 2/4 | 2 | 4 |
| check_understanding | 4 | 4/4 | 1 | 1 |
| safety_net | 4 | 4/4 | 2 | 2 |
| invite_questions | 4 | 2/4 | 1 | 3 |

## Disagreements (binary)

### backpain_visit · respond_emotion — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: That's a really understandable worry, especially with your uncle — I'd probably have been googling too.
- judge raw: `RESULT: done
EVIDENCE: "That's a really understandable worry, especially with your uncle — I'd probably have been googling too."
TIP: None`

### backpain_visit · support_respect — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: That's a really understandable worry, especially with your uncle — I'd probably have been googling too.
- judge raw: `RESULT: done
EVIDENCE: "That's a really understandable worry, especially with your uncle — I'd probably have been googling too."
TIP: None`

### cough_visit · accurate_info — human: missed, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Meaning post-viral bronchial hyperreactivity most likely. Airways are irritable after the infection. It settles.
- judge raw: `RESULT: done
EVIDENCE: "Meaning post-viral bronchial hyperreactivity most likely. Airways are irritable after the infection. It settles."
TIP: none`

### cough_visit · avoid_interrupting — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it. Anything else?
- judge raw: `RESULT: done
EVIDENCE: "I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it. Anything else?"
TIP: None`

### cough_visit · explain_exam — human: missed, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Shirt up, let me listen. Big breaths. In. Out. Again. Right. Chest's clear.
- judge raw: `RESULT: done
EVIDENCE: Shirt up, let me listen. Big breaths. In. Out. Again. Right. Chest's clear.
TIP: None`

### cough_visit · explore_complaint — human: missed, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Cough, is it?", "Any fever?", "Coughing anything up? Phlegm? What colour?", "Short of breath? Chest pain?", "Any blood in what you cough up?
- judge raw: `RESULT: done
EVIDENCE: "Cough, is it?", "Any fever?", "Coughing anything up? Phlegm? What colour?", "Short of breath? Chest pain?", "Any blood in what you cough up?"
TIP: None`

### cough_visit · invite_questions — human: missed, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Anything else?
- judge raw: `RESULT: done
EVIDENCE: "Anything else?"
TIP: None`

### cough_visit · shared_plan — human: partial, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it.
- judge raw: `RESULT: done
EVIDENCE: "I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it."
TIP: None`

### cough_visit · support_respect — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it.
- judge raw: `RESULT: done
EVIDENCE: "I'll put you on a salbutamol inhaler, two puffs when it's bad. Pharmacy will show you how to use it."
TIP: None`

### cough_visit · what_else — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Anything else?
- judge raw: `RESULT: done
EVIDENCE: "Anything else?"
TIP: None`

### headache_visit · accurate_info — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: These have all the hallmarks of the ordinary sort of headache that comes with stress and screens and bad sleep. Nothing you've said rings any alarm bells for me.
- judge raw: `RESULT: done
EVIDENCE: "These have all the hallmarks of the ordinary sort of headache that comes with stress and screens and bad sleep. Nothing you've said rings any alarm bells for me."
TIP: none`

### headache_visit · avoid_interrupting — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: Go on — anything else you've noticed about them?
- judge raw: `RESULT: done
EVIDENCE: "Go on — anything else you've noticed about them?"
TIP: None`

### headache_visit · explore_perspective — human: partial, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: My colleague's sister had a brain tumour and it started with headaches, and I know that's silly but it's two in the morning and you start thinking.
- judge raw: `RESULT: done
EVIDENCE: "My colleague's sister had a brain tumour and it started with headaches, and I know that's silly but it's two in the morning and you start thinking."
TIP: None`

### headache_visit · invite_questions — human: partial, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: Anyway — was there anything else, lovely?
- judge raw: `RESULT: done
EVIDENCE: "Anyway — was there anything else, lovely?"
TIP: None`

### headache_visit · respond_emotion — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: It's not silly at all, it's completely human — everyone goes there at two in the morning. I hear you.
- judge raw: `RESULT: done
EVIDENCE: "It's not silly at all, it's completely human — everyone goes there at two in the morning. I hear you."
TIP: None`

### headache_visit · shared_plan — human: partial, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: Be kind to yourself, mainly. Try to ease off the screens where you can, drink some more water, maybe fewer coffees.
- judge raw: `RESULT: done
EVIDENCE: "Be kind to yourself, mainly. Try to ease off the screens where you can, drink some more water, maybe fewer coffees."
TIP: None`

### headache_visit · what_else — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Anyway — was there anything else, lovely?
- judge raw: `RESULT: done
EVIDENCE: "Anyway — was there anything else, lovely?"
TIP: None`

### rash_visit · avoid_interrupting — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: Um, I thought maybe the new washing powder? We switched brands around then. But I switched back two weeks ago and it's still happening. Hot showers definitely make the itching worse after.
- judge raw: `RESULT: done
EVIDENCE: "Um, I thought maybe the new washing powder? We switched brands around then. But I switched back two weeks ago and it's still happening. Hot showers definitely make the itching `

### rash_visit · explore_perspective — human: partial, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Some people worry about steroid creams — how do you feel about that?
- judge raw: `RESULT: done
EVIDENCE: "Some people worry about steroid creams — how do you feel about that?"
TIP: None`

### rash_visit · intro_self — human: missed, judge: met (judge over-credits) — verifier confirmed (still wrong)
- judge evidence: Hi there, I'm Dr Marsh
- judge raw: `RESULT: done
EVIDENCE: Hi there, I'm Dr Marsh
TIP: none`

### rash_visit · set_tone — human: missed, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: Hi there, I'm Dr Marsh — sorry, the system's running slow today.
- judge raw: `RESULT: done
EVIDENCE: "Hi there, I'm Dr Marsh — sorry, the system's running slow today."
TIP: None`

### rash_visit · support_respect — human: partial, judge: met (judge over-credits) — verifier REJECTED (fixed)
- judge evidence: It hasn't come back because of anything you did wrong; it often resurfaces in adults, especially with stress or in winter.
- judge raw: `RESULT: done
EVIDENCE: "It hasn't come back because of anything you did wrong; it often resurfaces in adults, especially with stress or in winter."
TIP: None`

## Human-na rows (excluded)

- headache_visit · explain_exam (judge said missed)
