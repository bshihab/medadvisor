...
| Job | Model | Why |
|---|---|---|
| **Rubric scoring (LLM)** | **Qwen 3.5-4B** (Q4, ~3.0 GB, thinking off) — the only model since 2026-09-17 | Runs on llama.cpp. Its predecessor, Qwen 2.5-7B-Instruct, replaced MedGemma 4B, which rubber-stamped (**53% over-score**). A later 240-decision set — 5× the resolution of the first — then showed **Qwen 3.5-4B ahead of the 7B, 85.4% vs 79.2%**, on a 30% smaller download. Every judge-quality measurement since August ([`calibration/FINDINGS.md`](tools/llm-benchmark/calibration/FINDINGS.md)) is on the 4B, so the 7B was removed (the app deletes its file from upgraded phones); the director's gold scores remain the clinical check. |
