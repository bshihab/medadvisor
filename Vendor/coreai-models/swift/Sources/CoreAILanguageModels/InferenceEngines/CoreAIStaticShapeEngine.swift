    ...
    public func generate(
        ...
        if history.count > 0 {
            ...
            if commonPrefix < input.count && commonPrefix < history.count {
                ...
                history.clear()
                // MEDADVISOR PATCH: the reset above discards the entire KV
                // cache, so none of the matched prefix is reused. Reporting
                // commonPrefix here (as upstream does) claims a cache benefit
                // the engine just threw away.
                lastPrefixHitCount = 0
            } else {
                if processedTokenCount >= input.count {
