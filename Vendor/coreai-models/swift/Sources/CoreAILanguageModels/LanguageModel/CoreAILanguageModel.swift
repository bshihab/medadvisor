    ...
    public struct CoreAIExecutor: LanguageModelExecutor {
        ...
        private func respondVanilla(
            ...
            await channel.send(
                .response(
                    action: .updateUsage(
                        // MEDADVISOR PATCH: upstream hardcodes cachedTokenCount
                        // to 0, which silently zeroes Usage.Input.cachedTokenCount
                        // for every caller. Report the engine's actual prefix-cache
                        // hit for THIS generation (0 after a divergence reset —
                        // see the engine patches).
                        input: .init(
