...
| **LLM** | **Qwen 3.5-4B Q4_K_M** (GGUF, ~3.0 GB, thinking suppressed) via llama.cpp on Metal — the only model since 2026-09-17 (Qwen2.5-7B-Instruct removed; a one-time launch sweep deletes its file from upgraded installs). Greedy/deterministic sampling, `n_ctx` 6144 | Chosen on measured agreement, not the "med" label. The 7B replaced MedGemma 4B (96% accuracy / 3.3% over-score vs 53% over-score on the first 48-decision set); the 240-decision set then put Qwen 3.5-4B ahead of the 7B, 85.4% vs 79.2% (`tools/llm-benchmark/MODEL-COMPARISON.md`), and all calibration work since (`tools/llm-benchmark/calibration/FINDINGS.md`) is on the 4B, measured in the shipping llama.cpp stack. *(Supersedes the original Gemma 3n plan.)* |
...
- [x] **Build + smoke the verifier port on the Air** *(done 2026-09-07: verify = 39s vs scoring 101s = 39%, keep; found cloud-rubric missing aggregate flags — see FINDINGS)*: pull, build, run one consultation, watch `[Verify]` lines in the console; confirm a token "anything else?" gets rejected. The shipped-config evidence (84.1% vs gold) rides on this port working on-device.
